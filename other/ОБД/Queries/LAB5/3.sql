...
BEGIN
    ...
    ОСНОВНА ІНФОРМАЦІЯ
' ||
                     'Клієнт: ' || v_client_full_name || '
' ||
                     'Назва справи: ' || v_case_title || '
' ||
                     'Тип справи: ' || v_case_type_name || '

' ||
                     '            ВИТРАТИ
' ||
                     'Вартість справи: ' || v_case_type_price::TEXT || ' грн.
' ||
                     'Додаткові витрати: ' || v_additional_expenses::TEXT || ' грн.
' ||
                     'Загальна сума до сплати: ' || v_total_amount::TEXT || ' грн.
                     ';
    
    RAISE NOTICE '%', v_notice_text;
END;
$$;
