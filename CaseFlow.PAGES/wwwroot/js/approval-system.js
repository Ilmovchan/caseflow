...
async function handleApproval(id, type, action) {
    ...
    try {
        // Show loading state
        if (actionCell) {
            actionCell.innerHTML = '<span class="text-muted"><i class="fas fa-spinner fa-spin"></i> Обробка...</span>';
        }
        ...
        // Update the UI
        updateApprovalStatus(row, result.newStatus, result.newStatusText, result.newStatusColor);
        
        // Show success message
        showNotification(`${action === 'approve' ? 'Схвалено' : 'Відхилено'} успішно!`, 'success');
        ...
        // Show error message
        showNotification(`Помилка при ${action === 'approve' ? 'схваленні' : 'відхиленні'}: ${error.message}`, 'error');
    }
}
